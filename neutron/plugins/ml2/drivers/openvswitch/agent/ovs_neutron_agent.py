...
class OVSNeutronAgent(l2population_rpc.L2populationRpcCallBackTunnelMixin,
    ...
    def port_update(self, context, **kwargs):
        ...
        if self.enable_vip and kwargs.has_key('is_vip') and kwargs['is_vip']:
            port = self.plugin_rpc.get_device_details_for_network_vip(self.context,
                                                                      port_id,
                                                                      self.agent_id,
                                                                      self.conf.host)
            if not port:
                LOG.debug("get vip port detail failed %s", port_id)
                return
            ...
            if port_id not in self.vip_detail:
                ...
                if net_uuid not in self.network_vip:
                    ...
                    if tun_id in self.groups:
                        ...
                        group +=  ',type=select,selection_method=hash,fields(%s)' % (self.to_idc_lb_hash_fields)
    ...
    def port_delete(self, context, **kwargs):
        ...
        if port_id in self.vip_detail and self.enable_vip:
            ...
            LOG.debug("delete vip port %s", port_id)
            mac = port_dict['mac_address']
            vlan = port_dict['vlan']
            tun_id = port_dict['segmentation_id']
            ...
            if (net_uuid in self.network_vip and
                port_id in self.network_vip[net_uuid]):
                self.network_vip[net_uuid].remove(port_id)
                if len(self.network_vip[net_uuid]) == 0:
                    self.tun_br.delete_group(tun_id)
        ...
        try:
            ...
            if network_type in constants.TUNNEL_NETWORK_TYPES and self.enable_vip:
                vip_ports = self.plugin_rpc.get_network_vip_ports(self.context,
                                                                  net_uuid)
                if vip_ports:
                    LOG.debug("get vip port %s", vip_ports)
        ...
        if lvm.network_type in constants.TUNNEL_NETWORK_TYPES:
            ...
            if net_uuid in self.network_vip and self.enable_vip:
                for port_id in self.network_vip[net_uuid]:
                    ...
                    self.vip_detail.pop(port_id, None)
                self.network_vip.pop(net_uuid, None)
                self.tun_br.delete_group(lvm.segmentation_id)
    ...
    def cleanup_stale_flows(self):
        ...
        if self.enable_vip:
            LOG.info("Cleaning stale %s group table", self.tun_br.br_name)
            for group_id in self.groups:
                self.tun_br.delete_group(group_id)
